...
fn clamp_grad(x: f32, min: f32, max: f32) -> f64 {
    // Match the reference clamp semantics: gradients pass through values exactly
...
pub(crate) fn batch_loss_and_grad(
    ...
    for (column, &column_seq_len) in seq_lens.iter().enumerate().take(batch) {
        let column_seq_len = column_seq_len.min(seq_len);
...
mod tests {
    ...
    #[test]
    fn clamp_grad_matches_reference_boundary_semantics() {
